...
#
# fibo_4 se rapproche de la courbe normalisée de x**log2(3)
#
# Exercice 2.3
#
...
def puissance_matrice_2_2_ops (M, n) :
  ...
  if n==1:
    return M,0
  tmp,ops1= puissance_matrice_2_2_ops(M, n // 2)
  carre,ops2 = produit_matrice_2_2_ops(tmp, tmp)
  if n % 2 == 0:
    return carre,ops1+ops2
  else:
    res,ops3=produit_matrice_2_2_ops(M,carre)
    return res,ops1+ops2+ops3
...
def decompte_naif (a, b, c, d, e) :
  ...
  #return nbOfBits(b*c)+nbOfBits(d*e)
  return nbOfBits(b)*nbOfBits(c)+nbOfBits(d)*nbOfBits(e)
...
def produit_matrice_2_2_bits (M1, M2, compte = decompte_naif) :
  ...
  res[0][0] = M1[0][0] * M2[0][0] + M1[0][1] * M2[1][0]
  ops+=compte(0,M1[0][0],M2[0][0],M1[0][1],M2[1][0])
  res[1][0] = M1[1][0] * M2[0][0] + M1[1][1] * M2[1][0]
  ops+=compte(0,M1[1][0],M2[0][0],M1[1][1],M2[1][0])
  res[0][1] = M1[0][0] * M2[0][1] + M1[0][1] * M2[1][1]
  ops+=compte(0,M1[0][0],M2[0][1],M1[0][1],M2[1][1])
  res[1][1] = M1[1][0] * M2[0][1] + M1[1][1] * M2[1][1]
  ops+=compte(0,M1[1][0],M2[0][1],M1[1][1],M2[1][1])
...
def puissance_matrice_2_2_bits (M, n, compte = decompte_naif) :
  ...
  if n == 1:
    return M, 0
  tmp, ops1 = puissance_matrice_2_2_bits(M, n // 2)
  carre, ops2 = produit_matrice_2_2_bits(tmp, tmp)
  if n % 2 == 0:
    return carre, ops1 + ops2
  else:
    res, ops3 = produit_matrice_2_2_bits(M, carre)
    return res, ops1 + ops2 + ops3
...
def fibo_4_bits(n, compte = decompte_naif) :
  ...
  if n <= 2: return 1, 0
  M = [[1, 1], [1, 0]]
  N, ops = puissance_matrice_2_2_bits(M, n - 1)
  return N[0][0], ops
